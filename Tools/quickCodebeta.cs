...
namespace Tools
{
    ...
    public partial class quickCodebeta : Form
    {
        #region Filed
        ...
        private void OpenFile(string fileName)
        {
            ...
            editor.Font = new System.Drawing.Font("微软雅黑", 12);
            editor.Encoding = Encoding.UTF8;
            editor.OpenFile(fileName);
            editor.Document.FoldingManager.FoldingStrategy = new MingHTMLFolding();
            editor.Document.HighlightingStrategy = HighlightingStrategyFactory.CreateHighlightingStrategy("HTML");
        ...
        private void cesToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            CodeTextEditor editor = getNowTextEditor();
            ...
            try
            {
                ...
                {
                    editor.ActiveTextAreaControl.TextArea.InsertString($"<%={item.Text}.Replace(\"\\r\\n\",\"<br/>\")%>");
                }
                else
                {
                    editor.ActiveTextAreaControl.TextArea.InsertString($"<%={item.Text}%>");
                }
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// 获取现在选中的页面中的CodeTextEditor
        /// </summary>
        /// <returns></returns>
        public CodeTextEditor getNowTextEditor()
        {
            var tabpage = tabControl1.SelectedTab as TabPage;
            if (tabpage == null) return null;
            var editor = new CodeTextEditor();
            foreach (Control c in tabpage.Controls)
            {
                if (c is CodeTextEditor)
                {
                    editor = c as CodeTextEditor;
                }
            }
            return editor;
        }
    ...
    public class CodeTextEditor : TextEditorControl
    {
        ...
        public void LoadFile(Stream fs)
        {
            using (StreamReader reader = FileReader.OpenStream(fs, this.Encoding ?? Encoding.UTF8))
            {
